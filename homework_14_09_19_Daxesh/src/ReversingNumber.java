...
public class ReversingNumber {
    ...
    public static void isPalindrome(int number) {
        ...
        if (number != 0) {
            while (number != 0) {
                rem = number % 10;
                reverse_no = rem + (reverse_no * 10);
                number = number / 10;
            }
            if (compare == reverse_no) {
                System.out.println("true," + compare + " is a palindrome no.");
            } else {
                System.out.println("false," + compare + " is not a palindrome no.");
            }
        }
